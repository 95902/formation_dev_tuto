...
class MarkdownFormatterTest extends TestCase
{
    ...
    public function test_it_formats_an_answer_with_several_citations(): void
    {
        $expected = "**Question :** Comment declarer un sinistre auto ?\n\n"
            ...
            ."\n_[contexte ~1234 tokens]_";

        $this->assertSame($expected, (new MarkdownFormatter)->format($this->answerWithSeveralCitations()));
    }
    ...
    public function test_it_uses_the_singular_when_there_is_one_citation(): void
    {
        $expected = "**Question :** Comment declarer un sinistre auto ?\n\n"
            ...
            ."**1 source :**\n"
            ."- `wiki/delais.md:3` — Reponse breve\n"
            ."\n_[contexte ~42 tokens]_";

        $this->assertSame($expected, (new MarkdownFormatter)->format($this->answerWithOneCitation()));
    }
    ...
    public function test_it_reports_when_there_are_no_citations(): void
    {
        $expected = "**Question :** Comment declarer un sinistre auto ?\n\n"
            ."Il faut contacter votre assureur sous 5 jours ouvres.\n\n"
            ."Aucune source.\n"
            ."\n_[contexte ~10 tokens]_";

        $this->assertSame($expected, (new MarkdownFormatter)->format($this->answerWithNoCitations()));
    }
}
